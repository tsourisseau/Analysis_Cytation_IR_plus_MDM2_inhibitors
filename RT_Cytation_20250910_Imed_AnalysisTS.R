...
####################################################
#Analysis of Imed's results "RT_Cytation_20250910"
#Reading 6 days after IR
####################################################
...
setwd("//cifs/partage_sarcome/Equipe Italiano/Epigénétique_Tony/MDM2_IR/RT_Cytation_20250910_Imed_AnalysisTS")



#initialise experiment parameters
dose <- list("0Gy", "0.5Gy", "1Gy", "2Gy", "4Gy")
ttt <- list("KTX169", "BI907828")
Conc_KTX169 <- rep(x = c(0, 1.024E-3, 5.12E-3, 0.0256, 0.128, 0.64, 3.2, 16, 80, 400, 0),
                   times = 3)
Conc_BI907828 <- rep(x = c(0, 0, 1.28E-3, 6.4E-3, 0.032, 0.16, 0.8, 4, 20, 100, 500),
                     times = 3)
cell_lines <- list("IB111", "IB114", "IB115", "IB128", "IB147")
replicates <- 3
#read the 5 .txt files and save as variables
#The 5 .xls files (5 x 384-well plates) were saved as .txt files in one directory
List_Data_files <- list.files("\\\\cifs/partage_sarcome/Equipe Italiano/Epigénétique_Tony/MDM2_IR/RT_Cytation_20250910_Imed_AnalysisTS/RT_Cytation_20250910_Imed_AnalysisTS_RawData/RT_Cytation_20250910_Imed_AnalysisTS_RawData_corrected")
for (i in c(1:length(List_Data_files))) {
  #fetch data from file
  Data_nGy <- read.table(file = paste0("RT_Cytation_20250910_Imed_AnalysisTS_RawData/RT_Cytation_20250910_Imed_AnalysisTS_RawData_corrected/", 
                                      List_Data_files[i]),
                         sep = "\t",
                         na.strings = "NA")
  #generate data name
...
save(RawData, file = "RawData_RT_Cytation_20250910_Imed.RData")
...
#(pool the 2 "0nM" points for BI907828 and for KTX169, 4N each = 16 points for each dose (Gy) for each cell line)
...
for (i_ttt in c(1:length(ttt))) {   # ttt KTX-169 and BI907828
  for (i_dose in c(1:5)){           # doses Gy
    ...
    LNum <- 2 #first line used in 384-well plate
    for (i_cell_line in c(1:length(cell_lines))){ #cell lines
      ...
      for (i_replicate in c(1:3)){ #replicates
        ...
        LNum <- LNum+1
        if (i_dose==1){   # i.e. 0Gy                           # for the 0Gy doses, collect value for each cell line for standardisation
...
# pool 0nM for standardisation from the 2 ttt
i_cell_line <- NULL
cell_line_0nM_0Gy_STD_temp <- c()
for (i_cell_line in c(1:length(cell_lines))){
  cell_line_0nM_0Gy_STD_temp <- c(get(paste0(cell_lines[i_cell_line], 
                                                "_KTX169_0nM_0Gy_STD")),
                             get(paste0(cell_lines[i_cell_line], 
                                                "_BI907828_0nM_0Gy_STD"))
                             )
  
  name_cell_line_0nM_0Gy_STD_temp <- paste0(cell_lines[i_cell_line], "_0nM_0Gy_STD")
  assign(x = name_cell_line_0nM_0Gy_STD_temp, value = cell_line_0nM_0Gy_STD_temp)
...
dose_response_all_0nM <- rep(c(0, 0.5, 1, 2, 4), # 5 doses (Gy)
...
list_colours <- c("darkgreen", "red", "black", "blue", "grey")
names(list_colours) <- cell_lines
#linear y-scale
i_colour <- 1
for (i_column in names(data_0nM_AllDose)[-1]){
  color <- list_colours[i_colour]
  print(
    ggplot(data = data_0nM_AllDose) +
      scale_y_continuous(limits = c(0, 1.2)) +
    geom_point(aes(x = dose_response_all_0nM,
                   y = .data[[i_column]]),
                   color = color, size = 3, alpha = 0.3) +
    geom_smooth(aes(x = dose_response_all_0nM,
                    y = .data[[i_column]]),
                   color = color)
    )
  i_colour <- i_colour + 1
...
for (i_column in names(data_0nM_AllDose)[-1]){
  color <- list_colours[i_colour]
  print(
    ggplot(data = data_0nM_AllDose, aes(x = dose_response_all_0nM,
                                        y = .data[[i_column]])) +
      scale_y_log10(limits = c(0.01, 1.5)) +
      geom_point(color = color, size = 3, alpha = 0.3) +
...
##############################################################################
# log10 y scale creating ggplot objects
i_colour <- 1
for (i_column in names(data_0nM_AllDose)[-1]){
  color <- list_colours[i_colour]
  plot_temp <- ggplot(data = data_0nM_AllDose, aes(x = dose_response_all_0nM,
                                        y = .data[[i_column]])) +
          scale_y_log10(limits = c(0.01, 1.5)) +
          geom_point(color = color, size = 3, alpha = 0.3) +
          geom_smooth(method = "lm")
# assign plot
  plot_name <- paste0("plot", i_column, "noBI")
  assign(plot_name, plot_temp)
  i_colour <- i_colour + 1
  # print plot
  print(plot_temp)
}
...
##############################################################################
# create a function plotting one column
overlay_plot <- function(data, column) {
  print(
    ggplot(data = data, aes(x = dose_response_all_0nM,
                            y = column)) +
      scale_y_log10(limits = c(0.01, 1.5)) +
      geom_point(color = color, size = 3, alpha = 0.3) +
      geom_smooth(method = "lm")
...
#overlay_plot(data_0nM_AllDose, IB111_0nM_AllDose)
# make the function loop over the 5 columns
for (i_column in names(data_0nM_AllDose)[-1]){
  overlay_plot(data = data_0nM_AllDose, column = data_0nM_AllDose[[i_column]])
}
##############################################################################

# plot 0nM for the 5 cell lines
ggplot(data = data_0nM_AllDose) +
  scale_y_log10(limits = c(0.01, 1.5)) +
...
ggplot(data = matrix_KTX169_No_0) +
  ...
  geom_smooth(method = drm, method.args = list(fct = L.4()), se = FALSE, colour = "red") +
  scale_x_log10()

# KTX169
list_colours <- c("darkgreen", "red", "black", "blue", "grey")
names(list_colours) <- cell_lines
# Function to plot dose responses
# Build function with args = TTT, cell line 
plot_DoseResp <- function(n) {
  cell_line <- cell_lines[n]
  color <- names(list_colours[n])
  column_0Gy <- paste0("col_", cell_line, "_KTX169_", "0Gy_3N")
  column_0.5Gy <- paste0("col_", cell_line, "_KTX169_", "0.5Gy_3N")
  column_1Gy <- paste0("col_", cell_line, "_KTX169_", "1Gy_3N")
  column_2Gy <- paste0("col_", cell_line, "_KTX169_", "2Gy_3N")
  column_4Gy <- paste0("col_", cell_line, "_KTX169_", "4Gy_3N")
  print(
    ggplot(data = matrix_KTX169_No_0) + 
      
      scale_x_log10() +
    
      geom_point(aes(x = KTX169,
                     y = .data[[column_0Gy]],
                   color = "red")) +
      geom_smooth(aes(x = KTX169,
                      y = .data[[column_0Gy]],
                      color = "red"), 
...
plot_DoseResp <- function(n) {
  cell_line <- cell_lines[n]
  color <- names(list_colours[n])
  column_0Gy <- paste0("col_", cell_line, "_BI907828_", "0Gy_3N")
  column_0.5Gy <- paste0("col_", cell_line, "_BI907828_", "0.5Gy_3N")
  column_1Gy <- paste0("col_", cell_line, "_BI907828_", "1Gy_3N")
  column_2Gy <- paste0("col_", cell_line, "_BI907828_", "2Gy_3N")
  column_4Gy <- paste0("col_", cell_line, "_BI907828_", "4Gy_3N")
  print(
    ggplot(data = matrix_BI907828_No_0) + 
      
      scale_x_log10() +
      
      geom_point(aes(x = BI907828,
                     y = .data[[column_0Gy]],
                     color = "darkgreen")) +
      geom_smooth(aes(x = BI907828,
                      y = .data[[column_0Gy]],
                      color = "darkgreen"), 
                  method = drm, method.args = list(fct = L.4()), se = FALSE) +
...
plot_DoseResp <- function(n) {
  cell_line <- cell_lines[n]
  color <- names(list_colours[n])
  column_0Gy <- paste0("std0Gy_", cell_line, "_KTX169_", "0Gy", "_3N_std0Gy")
  column_0.5Gy <- paste0("std0Gy_", cell_line, "_KTX169_", "0.5Gy", "_3N_std0Gy")
  column_1Gy <- paste0("std0Gy_", cell_line, "_KTX169_", "1Gy", "_3N_std0Gy")
  column_2Gy <- paste0("std0Gy_", cell_line, "_KTX169_", "2Gy", "_3N_std0Gy")
  column_4Gy <- paste0("std0Gy_", cell_line, "_KTX169_", "4Gy", "_3N_std0Gy")
  print(
    ggplot(data = matrix_KTX169_No_0_std0Gy) + 
...
plot_DoseResp <- function(n) {
  cell_line <- cell_lines[n]
  color <- names(list_colours[n])
  column_0Gy <- paste0("std0Gy_", cell_line, "_BI907828_", "0Gy", "_3N_std0Gy")
  column_0.5Gy <- paste0("std0Gy_", cell_line, "_BI907828_", "0.5Gy", "_3N_std0Gy")
  column_1Gy <- paste0("std0Gy_", cell_line, "_BI907828_", "1Gy", "_3N_std0Gy")
  column_2Gy <- paste0("std0Gy_", cell_line, "_BI907828_", "2Gy", "_3N_std0Gy")
  column_4Gy <- paste0("std0Gy_", cell_line, "_BI907828_", "4Gy", "_3N_std0Gy")
  print(
